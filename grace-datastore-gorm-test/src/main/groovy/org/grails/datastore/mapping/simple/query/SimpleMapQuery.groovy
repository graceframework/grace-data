...
class SimpleMapQuery extends Query {
    ...
    private final SimpleMapEntityPersister entityPersister
    ...
    private List applyMaxAndOffset(List sortedResults) {
        final total = sortedResults.size()
        if (offset >= total) {
            return Collections.emptyList()
        }
    ...
    def associationQueryHandlers = [
            (AssociationQuery)        : { allEntities, Association association, AssociationQuery aq ->
                ...
                return executeAssociationSubQuery(datastore[getFamily(association.associatedEntity)],
                        association.associatedEntity, queryCriteria, aq.association)
            },
    ...
    def handlers = [
            ...
            (Query.EqualsAll)                : { Query.EqualsAll equalsAll, PersistentProperty property, Closure function = null, boolean onValue = false ->
                ...
                Assert.isTrue(values.every { property.type.isInstance(it) },
                        "Subquery returned values that are not compatible with the type of property '$name': $values")
            ...
            (Query.NotEquals)                : { Query.NotEquals equals, PersistentProperty property, Closure function = null, boolean onValue =
                    false ->
            ...
            (Query.IsNotNull)                : { Query.IsNotNull equals, PersistentProperty property, Closure function = null, boolean onValue =
                    false ->
            ...
            (Query.Between)                  : { Query.Between between, PersistentProperty property, Closure function = null, boolean onValue = false ->
                ...
                if (function != null) {
                    allEntities.findAll { function(resolveIfEmbedded(name, it.value)) >= from && function(resolveIfEmbedded(name, it.value)) <= to
                    }.collect { it.key }
                }
                else {
            ...
            (Query.GreaterThan)              : { Query.GreaterThan gt, PersistentProperty property, Closure function = null, boolean onValue =
                    false ->
                def name = gt.property
                final value = subqueryIfNecessary(gt)
                def allEntities = datastore[family]

                allEntities.findAll { (function != null ? function(resolveIfEmbedded(name, it.value)) : resolveIfEmbedded(name, it.value)) > value
                }.collect { it.key }
            },
            (Query.GreaterThanProperty)      : { Query.GreaterThanProperty gt, PersistentProperty property, Closure function = null, boolean onValue = false ->
                ...
                allEntities.findAll {
                    (function != null ? function(resolveIfEmbedded(name, it.value)) : resolveIfEmbedded(name, it.value)) > it.value[other] }.collect { it.key }
            },
    ...
    protected Object subqueryIfNecessary(Query.PropertyCriterion pc, boolean uniqueResult = true) {
        def value = pc.value
    ...
    protected PersistentProperty getValidProperty(criterion) {
        if (criterion instanceof Query.PropertyNameCriterion) {
            ...
            if (property == null) {
                ...
                if (identity.name == criterion.property) {
                    return identity
                }
                else {
    ...
    protected String getFamily(PersistentEntity entity) {
        ...
        if (table == null) {
            table = entity.getJavaClass().getName()
        }
        return table
    }
