...
public class AggregateTimestampProvider implements TimestampProvider {
    ...
    public <T> T createTimestamp(Class<T> dateTimeClass) {
        if (this.timestampProviders.size() > 1) {
            ...
            return createTimestamp(this.timestampProviders.iterator().next(), dateTimeClass);
        }
        throw new IllegalArgumentException("dateTimeClass given as parameter isn't supported by any TimestampProvider. " +
                "You should call supportsCreating first.");
    }
