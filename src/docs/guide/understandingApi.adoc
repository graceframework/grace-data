...
The low-level API classes are located within the `grace-datastore-core` subproject, whereas the higher-level APIs used to enhance domain classes can be found in `grace-datastore-gorm`. In this section, we will discuss the low-level API.
